...
## Publishing to Remote Skill Store

When this skill is ready for the remote store:

```bash
# Preferred: use gh CLI for direct remote operations
cd ~/.hermes/skills/devops/hermes-install-tavily-cli
gh repo clone igamenovoer/houmao-agents /tmp/houmao-agents
cp -r . /tmp/houmao-agents/skillset/hermes-install-tavily-cli
cd /tmp/houmao-agents
git add skillset/hermes-install-tavily-cli
git commit -m "Add hermes-install-tavily-cli skill"
git push origin main
```

Do NOT use manual `git init` + `git push` workflows when `gh` is available.

## References

- `references/tavily-official-install.md` — condensed upstream docs, context isolation patterns, official skill catalog
